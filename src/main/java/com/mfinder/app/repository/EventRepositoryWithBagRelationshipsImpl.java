...
public class EventRepositoryWithBagRelationshipsImpl implements EventRepositoryWithBagRelationships {
    ...
    public Optional<Event> fetchBagRelationships(Optional<Event> event) {
        return event.map(this::fetchArtists);
    }
    ...
    public List<Event> fetchBagRelationships(List<Event> events) {
        return Optional.of(events).map(this::fetchArtists).orElse(Collections.emptyList());
    }

    Event fetchArtists(Event result) {
        return entityManager
            .createQuery("select event from Event event left join fetch event.artists where event is :event", Event.class)
            .setParameter("event", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Event> fetchArtists(List<Event> events) {
        ...
        List<Event> result = entityManager
            .createQuery("select distinct event from Event event left join fetch event.artists where event in :events", Event.class)
